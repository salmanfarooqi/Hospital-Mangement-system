...
const deleteAppointment = async (req, res) => {
  try {
    ...
    //
    const { doctorId, patientId } = appointmentDoc;

    // delete appointment id from patient's appointment array
    const patientDeleteAppointment = await Patient.updateOne(
      { _id: patientId },
      { $pull: { appointment: appointmentId } }
    );

    console.log("Patient : " + patientDeleteAppointment);

    // delete appointment id from doctor's appointment array

    // delete appointment id from patient's appointment array
    const doctorDeleteAppointment = await Doctor.updateOne(
      { _id: doctorId },
      { $pull: { appointment: appointmentId } }
    );

    console.log("Patient : " + doctorDeleteAppointment);
